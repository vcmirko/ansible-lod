...
# ==============================================================================
# DESCRIPTION
# The custom logic used to create a qtree
#
# It's deciding whether to create a quota and / or cifs share based on the metadata
# It's setting the quota target based on the volume & qtree name
# It's setting the SVM & volume names based on the primary SVM & volume names
# It's removing the environments that are not needed based on the service level
# Quota is only set on primary
# Shares are synced to all environments
#
# VERSION HISTORY
...
def run_module():
    ...
    is_quota                           = meta.get('is_quota') # True or False
    is_share                           = meta.get('is_share') # True or False
    ...
    try:
        ...
        if not is_quota:
            log("Removing quota from all environments")
            primary.pop('quota')
            backup_local.pop('quota')
            backup_remote.pop('quota')
            adpe.pop('quota')

        if not is_share:
            log("Removing share from all environments")
            primary.pop('share')
            backup_local.pop('share')
            backup_remote.pop('share')
            adpe.pop('share')
